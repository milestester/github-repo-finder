describe('$gitHubSearch', function () {
  ...
  it("tests the github search service for good response", function () {
  ...
  it("tests the github search service for 404 response", function () {
  ...
  it("tests the github search service for other response", function () {
    httpBackend.whenGET("https://api.github.com/users/miles25tester/repos?sort=updated").respond(400, badGitHubMockRes);
    gitHubSearch.userData("miles25tester").then(function(res) {
      expect(res.errorMessage).toEqual("Error Occured, Please Search Again");
    });
    httpBackend.flush();
  });
});
